...
/*
 * (PWP) External definitios for things used
...
{                                     /* Fill one packet buffer */
  ...
  *leftover = '\0';
  ...
  size = (dp - data);
  *dp = '\0';                                /* mark (current) the end. */
  debug(F111, "getpkt eof/eot", data, size); /* Fell thru before full, */
  return size;                      /* return partial filled last packet */
}
...
{                                       /* Set parameters */
  ...
  /*
   * (PWP) Save current send packet size
