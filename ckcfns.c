...
extern int spsiz, spmax, rpsiz, timint, srvtim, rtimo, npad, ebq, ebqflg, rpt,
    rptq, rptflg, capas, keep;
extern int pktnum, prvpkt, sndtyp, bctr, bctu, fmask, size, osize, maxsize,
    spktl, nfils, stdouf, warn, timef, spsizf;
extern int parity, speed, turn, turnch, delay, displa, pktlog, tralog, seslog,
    xflg, mypadn;
...
CHAR *rpar() {
  ...
  data[10] = tochar((atcapr ? atcapb : 0) | (lpcapr ? lpcapb : 0) |
                    (swcapr ? swcapb : 0));
...
openi(name) char *name;
{
  int x, filno;
  if (memstr)
    return (1); /* Just return if file is memory. */
...
clsof(disp) int disp;
{
  ...
  if ((x = zclose(ZOFILE)) < 0) { /* Try to close the file */
    ...
    if (*filnam)
      zdelet(filnam); /* and not keeping incomplete files */
