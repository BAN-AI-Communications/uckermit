...
/*
 * Packet buffer and window sizes, will probably
 * need to be #ifdef'd for each system.
 */
...
/*
 * (PWP) this is used to avoid gratuitous
 * function calls while encoding or decoding
 * a packet. The previous way involved two
 * nested function calls for EACH character
 * of the file. This way, we only do 2 calls
 * per K of data. This reduces packet encoding
 * time to 1% of the former cost.
 */

#define INBUFSIZE 1024 /* Size of the buffered file input & output buffer */

/* 
 * get/put the next file character;
 * like getc()/putc() macros
 */
