...
/*
 * Note -- if you change this file, please amend the version number
 * and date at the top of ckcfns.c accordingly.
 */
...
extern int czseen, nakstate, quiet;
...
input() {
  ...
  /*
   * If previous packet again, a timeout pseudopacket,
  ...
  ttflui();                             /* Got what we want, clear buffer */
...
/*
 * (PWP) recalculate the optimal packet length in the face of
 * errors. This is a modified version of the algorithm by
 * John Chandler in Kermit/370; see "Dynamic Packet Size Control",
 * Kermit News, V2 #1, June 1988.
 *
 ...
 *
 * Differentiate with respect to number of chars,
 *  solve for packet_len, get:
 *
...
{                                      /* Send Attributes */
  ...
  if (zsattr(&x) < 0)
    return -1;                         /* Get attributes or die trying */
  i = 0;
  data[i++] = '.';                     /* System type */
...
{                                   /* Read response to attribute packet */
  ...
  if (*s == 'N')
    return -1;                      /* then other Kermit is refusing. */
  return 0;
}
...
#define SPBUFL 512                  /* System-dependent parameters */
  ...
  if ((l > 0) && (l > zfree(filnam))) {
    cxseen = 1;                     /* Set to true; file will be deleted */
